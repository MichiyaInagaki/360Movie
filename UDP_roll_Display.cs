...
public class UDP_roll_Display : MonoBehaviour
{
    ...
    private float initial_roll = 0.0f;
    //
    ...
    void Update()
    {
        ...
        roll_val = float.Parse(dest[0]);
        pitch_val = float.Parse(dest[1]);
        yaw_val = -float.Parse(dest[2]);
        // ローパスフィルタ
        ...
        if (Input.GetKeyDown(KeyCode.R))
        {
            ...
            initial_roll = roll_val;
        }
        yaw_val = yaw_val - initial_yaw;
        pitch_val = pitch_val - initial_pitch;
        roll_val = roll_val - initial_roll;
        Debug.Log("roll: " + roll_val + " pitch: " + pitch_val + " yaw: " + yaw_val);	//送られてきたデータの表示
    }
}
